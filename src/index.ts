...
class StreamableHttpTransport implements Transport {
  ...
  start(): Promise<void> {
    ...
    // We use application/json to bypass strict client content-type checks.
    ...
    // Listen for incoming streamed data from the client
    this.req.on('data', (chunk: Buffer) => {
      buffer += chunk.toString();
      
      // 1. Try to parse by newlines (Standard NDJSON)
      let newlineIndex;
      ...
      // 2. Fallback: If no newlines, try parsing the whole buffer as a complete JSON object.
      if (buffer.trim()) {
        try {
          const message = JSON.parse(buffer.trim()) as JSONRPCMessage;
          this.onmessage?.(message);
          buffer = ''; // Successfully parsed! Clear the buffer.
        } catch {
          // JSON is incomplete. Do nothing and wait for the next chunk of data.
        }
      }
    });

    this.req.on('end', () => {
      // Process any trailing data that didn't end in a newline
      if (buffer.trim()) {
        try {
          const message = JSON.parse(buffer.trim()) as JSONRPCMessage;
          this.onmessage?.(message);
        } catch {
          // Ignore parse errors at the very end
        }
      }
      void this.close();
    });
  ...
  send(message: JSONRPCMessage): Promise<void> {
    // Write JSON and force a newline to flush the chunk
    if (!this.res.writableEnded) {
...
async function startHttpTransport(mcpServerTemplate: McpServer, port: number = 8080) {
  ...
  // Keep a single active transport for SSE bypassing for the Inspector
  let activeSseTransport: SSEServerTransport | null = null;

  const server = http.createServer((req, res) => {
    // 1. CORS Headers for Inspector and cross-origin clients
    ...
    if (req.method === 'GET' && req.url === '/message') {
      void (async () => {
        try {
          ...
          activeSseTransport = transport; // Store for singleton bypass
          ...
          await connectionServer.connect(transport);
          log.info('Client connected via SSE');
        } catch (error) {
    ...
    // 6. ROUTE: SSE POST (Message Receiving)
      ...
      // Bypass session ID logic and route directly to the active transport
      ...
      try {
        ...
        req.on('end', () => {
          void (async () => {
            try {
              // Ensure we trim to prevent unexpected end of JSON input errors
              const parsedBody = body.trim() ? JSON.parse(body.trim()) : undefined;
              await activeSseTransport!.handlePostMessage(req, res, parsedBody);
            } catch (error) {
  ...
  server.listen(port, '0.0.0.0', () => {
    ...
    log.info(` -> SSE Endpoint: GET /message (POST /message for replies)`);
